 ...
 * Same API shape as Open Food Facts, but for cosmetics / personal care products.
 * No authentication required.
 */
export const openBeautyFactsProvider: BarcodeProvider = {
  id: "open_beauty_facts",
  layer: "layer2_open_beauty_facts",

  async lookup({ normalized_barcode }) {
    const url = `https://world.openbeautyfacts.org/api/v2/product/${encodeURIComponent(normalized_barcode)}?fields=product_name,brands,quantity,categories,image_front_url,status`;
    ...
    try {
      ...
      return {
        ...
        provider: "open_beauty_facts" as const,
        layer: "layer2_open_beauty_facts" as const,
        error_code: code,
      };
    }
