...
import { lookupBarcode, createInventoryItem } from "./inventory";
import { createTransaction } from "./transactions";
import { processReceiptText } from "./receipts";
...
export async function ingestByBarcode(data: {
  ...
  const item = await lookupBarcode(data.barcode);

  if (!item) {
    return { success: false as const, error: "barcode_not_found" };
  }
