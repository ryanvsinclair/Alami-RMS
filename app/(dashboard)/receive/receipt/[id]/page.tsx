  ...
  useEffect(() => {
    ...
    getReceiptDetail(params.id)
      .then((data) => {
        ...
        // Default to photo tab if we have an image, digital otherwise
        if ((data as ReceiptDetail).signed_image_url) {
          setTab("photo");
        }
      })
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {!loading && !error && receipt && (
        <>
          {/* Tab toggle -- only show if we have both views */}
          ...
          {/* Fallback: no photo, no line items */}
          {!hasPhoto && !hasLineItems && (
          ...
          {/* Only one view available -- no tab shown, render it directly */}
          {!hasPhoto && !hasLineItems ? null : null}
