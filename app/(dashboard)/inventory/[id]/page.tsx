...
export default function ItemDetailPage({
  ...
  const [level, setLevel] = useState<{ current_quantity: number; transaction_count: number } | null>(null);
    ...
    <>
      ...
      <div className="p-4 space-y-4">
        ...
        {item.barcodes.length > 0 && (
          <Card>
            <h3 className="text-sm font-semibold text-muted uppercase tracking-wide mb-2">Barcodes</h3>
            <div className="flex flex-wrap gap-1">
              {item.barcodes.map((b) => (
                <Badge key={b.id} variant="info">{b.barcode}</Badge>
              ))}
            </div>
          </Card>
        )}
              ...
              <Card key={tx.id}>
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className={`font-semibold ${Number(tx.quantity) >= 0 ? "text-green-600" : "text-red-600"}`}>
                      {Number(tx.quantity) >= 0 ? "+" : ""}{Number(tx.quantity)} {tx.unit}
                    </p>
                    {tx.unit_cost && (
