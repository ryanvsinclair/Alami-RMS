...
export default function ManualReceivePageClient() {
  const [step, setStep] = useState<Step>("search");
  const [search, setSearch] = useState("");
  const [items, setItems] = useState<ReceiveInventoryItemOption[]>([]);
  ...
  useEffect(() => {
    async function fetchItems() {
      ...
      setItems(result as ReceiveInventoryItemOption[]);
    }
    const timeout = setTimeout(fetchItems, 200);
    return () => clearTimeout(timeout);
  }, [search]);

  function selectExistingItem(item: ReceiveInventoryItemOption) {
